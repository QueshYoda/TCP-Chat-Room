...
def write():
    while True:
        ...
        if message[len(nickname) + 3: ].startswith('/'):
            print("girdi buraya")
            #burada string operasyoya çalınu sadece komutu almaşıyor dikkat et
            if nickname == 'admin':
                if message[len(nickname): ].find('/kick'):
                    client.send(f'KICK {message[len(nickname) + 2 + 6:]}'.encode('UTF-8'))
                    print(f'KICK {message[len(nickname) + 2 + 6:]}'.encode('UTF-8'))
                elif message[len(nickname) + 5: ].startswith('/ban'):
                    client.send(f'BAN {message[len(nickname) + 2 + 3:]}'.encode('UTF-8'))
                    print(f'KICK {message[len(nickname) + 2 + 3:]}'.encode('UTF-8'))
            else:
                print("hele hele kele bak sie mk")
